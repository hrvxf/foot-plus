  ...
  return (
    <main className="mx-auto max-w-6xl px-6 pb-16 pt-12">
      <section className="rounded-3xl border border-brand-sageLight/35 bg-linear-to-br from-white via-white to-brand-sageLight/20 p-7 shadow-sm md:p-10">
        ...
        <h1 className="mt-3 font-heading text-4xl font-semibold text-brand-sageDark md:text-5xl">
          Foot Health Advice and Guides
        </h1>
        <p className="mt-4 max-w-3xl text-lg leading-relaxed text-brand-charcoal/75">
          Practical, plain-English guidance to help you understand common foot concerns,
          routine foot care and when to seek professional support.
        </p>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_1fr]">
        {featuredArticle ? (
          <Link
            ...
            className="rounded-3xl border border-brand-sageLight/30 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-sageDark/70">
              Featured guide
            </p>
            <h2 className="mt-3 font-heading text-2xl font-semibold text-brand-sageDark">
              {featuredArticle.title}
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-brand-charcoal/75">
              {featuredArticle.description}
            </p>
            <p className="mt-4 text-sm font-semibold text-brand-sageDark">
              Read {featuredArticle.shortTitle}
            </p>
          </Link>
        ) : null}
        <AdviceSafetyNotice />
      ...
      <section className="mt-8">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {publishedAdviceArticles.map((article) => (
            <article
              key={article.slug}
              className="flex rounded-2xl border border-brand-sageLight/30 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-sageDark/25"
            >
              <div className="flex flex-col">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-sageDark/65">
                  {article.category}
                </p>
                {/* Keep the title dominant so the card reads as a guide index rather than a preview block. */}
                <h3 className="mt-2 font-heading text-[1.35rem] font-semibold leading-tight text-brand-sageDark">
                  <Link href={`/advice/${article.slug}`}>{article.shortTitle}</Link>
                </h3>
                <div className="mt-3 text-[11px] uppercase tracking-[0.14em] text-brand-charcoal/55">
                  <span>{article.readingTime}</span>
                </div>
                <Link
                  href={`/advice/${article.slug}`}
                  className="mt-auto pt-3 text-sm font-semibold text-brand-sageDark underline underline-offset-4"
                >
                  Read guide
                </Link>
              </div>
              ...
              <ul className="mt-2 space-y-1 text-sm">
                {publishedAdviceArticles
                  ...
                  .map((article) => (
                    <li key={article.slug}>
                      <Link
                        className="text-brand-charcoal/75 underline"
                        href={`/advice/${article.slug}`}
                      >
                        {article.shortTitle}
                      </Link>
