  ...
  return (
    <main className="mx-auto max-w-5xl px-6 pb-16 pt-8 md:pt-12">
      ...
      <div className="mt-8 space-y-6">
        ...
        <CardSection title={`Home visits across ${page.area}`}>{page.local.map(p=><p className="mb-3 last:mb-0" key={p}>{p}</p>)}<p className="mt-3">For nearby options, see <Link className="font-semibold text-brand-sageDark underline" href="/areas-we-cover">areas Foot+ covers</Link>{page.nearby.length ? <> or {page.nearby.map((n,i)=><span key={n.href}>{i>0 ? ", " : " "}<Link className="font-semibold text-brand-sageDark underline" href={n.href}>{n.title}</Link></span>)}</> : null}.</p></CardSection>
